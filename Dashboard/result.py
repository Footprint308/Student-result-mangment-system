...
class resultClass:
    def __init__(self, root):
        ...
        self.text_student.place(x=280,y=100,width=200)
        # self.text_student.current(0)
        ...
        txt_marks_ = Entry(self.root, textvariable=self.var_marks, font=("goudy old style",20,'bold'),bg='lightyellow').place(x=280,y=280,width=320)
        ...
        self.bg_img = self.bg_img.resize((630,350), filters[0])  # Change the index to use different filters
        self.bg_img = ImageTk.PhotoImage(self.bg_img)


        self.lbl_bg=Label(self.root,image=self.bg_img).place(x=660,y=100,width=630,height=350)
    ...
    def add(self):
        ...
        try:
            if self.var_name.get()=="":
                messagebox.showerror("Error","Please search from  student record",parent=self.root)
            else:
                cur.execute("select * from result where roll=? and course=?",(self.var_roll.get(), self.var_course.get(),))
                row=cur.fetchone()
                if row != None:
                    messagebox.showerror("Error","Result already exits",parent=self.root)
                else:
